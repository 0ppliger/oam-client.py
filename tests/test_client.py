...
def test_create_entity():
    emit = EmitterClient("http://localhost:8080")
...
def test_delete_edge_tag():
    ...
    t = emit.createEdgeTag(
        property=SourceProperty("delete", 100),
        edge=e.subject)

    emit.deleteEdgeTag(t.subject)

def test_update_edge_tag():
    emit = EmitterClient("http://localhost:8080")
